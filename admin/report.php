  ...
  <div class="wrapper d-flex align-items-stretch">
    ...
    <div id="content" class="p-4 p-md-5 pt-5">
      ...
      <div id="x">
        ...
        //`type`, `ongoing`, `disscuess`, `date

        // no`, `type`, `discuess`, `ongoing`, `date`

        if ($pmonth == 01) {

          $piyear = $year - 1;
          $sqlpreviusongoing1 = $db->prepare("SELECT * FROM recvie WHERE  MONTH(date)='$lastmonth' AND YEAR(date)='$piyear'");
          ...
          $sqlpreviusongoing1 = $db->prepare("SELECT * FROM recvie WHERE  MONTH(date)='$lastmonth' AND YEAR(date)='$year'");
          $sqlpreviusongoing1->execute();
          while ($rowp = $sqlpreviusongoing1->fetch()) {
            $aaaaa[] = $rowp['ongoing'];
            $bbbbb[] = $rowp['discuess'];
        ...
        $s = $db->prepare("SELECT * FROM recvie WHERE MONTH(date)='$pmonth' ORDER BY `no` DESC");
        $s->execute();
        while ($rowss = $s->fetch()) {
          $lmonth[] = $rowss['date'];
        }

        $tt = $db->prepare("SELECT * FROM type WHERE MONTH(date)='$pmonth' ORDER BY `no` DESC");
        $tt->execute();
        while ($rowtt = $tt->fetch()) {
          $llmonth[] = $rowtt['date'];
        }
        ...
        if ($pmonth == 01) {
          ...
          $sqlpreviusongoing11 = $db->prepare("SELECT * FROM type WHERE MONTH(date)='$lastmonth' AND YEAR(date)='$piyear'");
          ...
          $sqlpreviusongoing11 = $db->prepare("SELECT * FROM type WHERE  MONTH(date)='$lastmonth' AND YEAR(date)='$year'");
          $sqlpreviusongoing11->execute();
          while ($rowp2 = $sqlpreviusongoing11->fetch()) {
            $cc[] = $rowp2['ongoing'];
            $dd[] = $rowp2['disscuess'];
        ...
        <table class="table table-hover table-sm ">
          ...
          for ($i = 0; $i < count($domethod); $i++) {
            ...
            $sqlprolastmonthprocr1 = $db->prepare("SELECT COUNT(*) FROM doprocess WHERE mainmediatior !='' AND disputeRcvMethod = '$domethod[$i]' ");
            ...
            $ongoingfinal = $resultreglmonthprocr1;
            ...
            $nodisccussfinal = $resultreglmonthprocrempty1;
          ...
          ?>
            ...
            <tbody>
              <tr style="text-align: center">
                ...
                <td class="ff"> <?php echo $x //3total D 6
                                ?></td>
                <td class="ff"> <?php echo $resultprolmonth //settlment 7
                                ?></td>
                <td class="ff"> <?php echo $lastmonthsetmenttotal //non settlement 
                                ?></td>
                <td class="ff"> <?php echo $lastmonthsetmenttotalchair  // non settlment chairman 
                                ?></td>
                <td class="ff"> <?php echo $resultrowrlastmonthwidraw2 //reject 
                                ?></td>
                <td class="ff"> <?php echo $resultrowrlastmonthwidraw1 //widrawal 
                                ?></td>


                <td class="ff"> <?php echo $totalcompletecount //total complete  
                                ?></td>



                <td class="ff"> <?php echo  $ongoingfinal; //ongoing 
                                ?></td>
                <td class="ff"> <?php echo  $nodisccussfinal; //nodicess 
                                ?></td>
            ...
            <tr class="totcolor ">
              ...
              <td class="ff"> <?php echo $lmonthregistertotal //total register count
                              ?></td>
              <td class="ff"> <?php echo $xtotal ?></td>
              <td class="ff"> <?php echo $settletotal //settelment total 
                              ?></td>
              <td class="ff"> <?php echo $nonsettlementtotal // non sttlement total
                              ?></td>
              <td class="ff"> <?php echo $nonsettlementtotalcahirman  //non sttlement total chairman
                              ?></td>
              <td class="ff"> <?php echo $rejecttotal //reject 
                              ?></td>
              <td class="ff"> <?php echo $widrawaltotal //widthrawal 
                              ?></td>
              ...
              <td class="ff"> <?php echo $totalcomplete //total complete 
                              ?></td>

              <td class="ff"> <?php echo  $lmonthonggoing; //ongoing 
                              ?></td>
              <td class="ff"> <?php echo $lmonthnodiscuess //no disscuess 
                              ?></td>
            ...
            $typearray  = array(
              'Ntz;Lnkd;W fhak; tpistpj;jy; gpupT 314> 315> 325', 'Ntz;Lnkd;W fLq;fhak; tpistpj;jy; gpupT 316> 326',
            ...
            for ($i = 0; $i < count($typearray); $i++) {
              ...
              $ongoingfinaltype = $resulttypelmonthgoing;
              ...
              $nodisscussfinaltype = $resulttypenodiss;
            ...
            ?>
              ...
              <tr style="text-align:center">
                ...
                <td class="ff"> <?php echo $totallmonthregister //total register
                                ?></td>

                <td class="ff"> <?php echo $x1  ?></td>

                <td class="ff"> <?php echo $resultlmonthsettlement //settlwmtn 6:1  
                                ?></td>
                <td class="ff"> <?php echo $totaltypenonsett //nonsettlement 
                                ?></td>
                <td class="ff"> <?php echo $totaltypenonsetlmentchairman //viachairman non setl   
                                ?></td>
                <td class="ff"> <?php echo $resultlmonthreject //reject  
                                ?></td>
                <td class="ff"> <?php echo $resulttypelmonthwidrawal //widrwal   
                                ?></td>



                <td class="ff"> <?php echo  $totalcompletecounttype; //totalcomplete 
                                ?></td>
                <td class="ff"> <?php echo  $ongoingfinaltype; //nodisscuess 
                                ?></td>
                <td class="ff"> <?php echo  $nodisscussfinaltype; //ongoing 
                                ?></td>
              </tr>
              ...
              if ($ccdate == $last_day_this_month) {
                ...
                if ($llmonth[$i] != $now) {
                  $vv = "INSERT INTO type (ongoing,type,disscuess,date)VALUES('$ongoingfinaltype','$typearray[$i]','$nodisscussfinaltype','$now')";
                  $db->exec($vv);
                }
              }
            ...
            <tr class="totcolor">
              ...
              <td class="ff"> <?php echo $totalcomplete //total complete 
                              ?></td>
              <td class="ff"> <?php echo $lmonthonggoing //no disscuess 
                              ?></td>
              <td class="ff"> <?php echo  $lmonthnodiscuess; //ongoing 
                              ?></td>
            </tr>
